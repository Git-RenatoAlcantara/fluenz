...
A Solução: Itens consumíveis que afetam o gameplay e mecânica de Loja.

3.1 O Conceito (Catálogo)

A "Loja do Alquimista" é uma nova aba ou modal onde o usuário gasta suas Gems.

Itens Disponíveis:

Consumíveis (Potions & Scrolls):

🧪 Potion of Clarity (5 Gems): Dobra o XP ganho pelos próximos 30 minutos.

🧊 Cryo Crystal (10 Gems): "Congela" o streak. Se você não estudar amanhã, o streak não zera (consumo automático).

📜 Quest Reroll (2 Gems): Substitui uma missão diária difícil por outra.

Cosméticos (Permanentes):

Temas de UI: "Dark Necromancer" (Roxo/Preto), "High Elf" (Dourado/Branco).

Avatar Borders: Molduras animadas para o perfil.

3.2 Especificação Técnica (Fluxo de Compra e Uso)

Transação (Backend):

Verificar saldo de Gems do usuário.

Se saldo >= custo: Decrementar Gems e Adicionar Item ao Inventário (UserInventory).

Operação atômica (prisma.$transaction).

Aplicação de Efeitos (Logística):

XP Boost: Ao clicar em "Usar" no inventário, define user.activeEffects.xpBoostExpiresAt = now() + 30min. O backend checa essa data ao conceder XP.

Streak Freeze: Ao falhar o streak no cron job diário, o sistema checa se o usuário possui o item Cryo Crystal. Se sim, remove 1 item e mantem o streak.

3.3 Schema Prisma (Shop & Inventory)

Adicionar ao schema.prisma para suportar a loja:
...
model ShopItem {
  ...
  icon        String   // Nome do ícone Lucide ou caminho da imagem
  config      Json     // { "durationMinutes": 30, "multiplier": 2.0 } para lógica interna
...
model UserInventory {
  ...
  isEquipped  Boolean  @default(false) // Para temas/borders
...
// Atualizar User para suportar efeitos ativos
model User {
  // ... campos existentes ...
  activeEffects Json?  // Ex: { "xpBoostExpiresAt": "2023-10-27T10:30:00Z", "streakFreeze": true }
}


3.4 Interface (UI Components)

ShopModal: Grid de cards com ícone, preço (em Gems) e botão "Comprar".

InventoryDrawer: Lista de itens comprados. Botão "Usar" para poções, "Equipar" para temas.

Active Effects HUD: Pequenos ícones circulares perto da barra de Mana mostrando buffs ativos com contagem regressiva (ex: "2x XP: 14min").
